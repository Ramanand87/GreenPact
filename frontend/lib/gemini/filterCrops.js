...
const getChatSession = () => {
  if (!currentChat) {
    ...
    currentChat = genAI.chats.create({
      ...
      config: {
        systemInstruction: `
You are a distance filtering assistant. Your ONLY job is to filter crop data based on a maximum driving distance from an origin location.
...
"origin": "The starting location provided",
 ...
 "matched": [
  {
   "crop_id": "The original crop ID",
   "crop_name": "The original crop name",
   "location": "The original crop location",  
...
function getCacheKey(originLocation, crops, maxDistanceKm) {
  const cropHash = crops.map(c => c.location).sort().join('|');
  return `${originLocation}:${maxDistanceKm}:${cropHash}`;
}

export async function filterCropsByDistance(
  originLocation,
  crops,
  maxDistanceKm 
) {
  const cacheKey = getCacheKey(originLocation, crops, maxDistanceKm);
...
dataList: ${JSON.stringify(crops.map((c) => ({
  crop_id: c.crop_id,
  ...
  location: c.location
    ...
    const matchedCropsWithFullData = parsedJson.matched.map((match) => {
      const originalCrop = crops.find(c => c.crop_id === match.crop_id);
      if (originalCrop) {
        return {
          ...originalCrop,
    ...
    const result = {
      ...
      matched: matchedCropsWithFullData,
    };
    ...
    if (error.message?.includes('429') || error.message?.includes('exhausted') || error.message?.includes('limit')) {
      ...
      return {
        ...
        maxDistanceKm: maxDistanceKm,
        matched: crops, // Return all crops as fallback
        fallback: true,
        error: "API rate limited, showing all crops",
      };
    }
